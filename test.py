...
from api.Ticket_Sectors.Domestic.airlines.yeti_air import YetiAir
cookies = {
...
def Search(dd,mm,yy,fromsector,tosector,passenger_count):
  session = YetiAir().get_logged_in_session()
  headers = {
      'Connection': 'keep-alive',
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.71 Safari/537.36 Edg/94.0.992.38',
      'Content-Type': 'application/json; charset=UTF-8',
...
class Fetch():
  ...
  def get_flight_and_seats(self,fare="NPR"):
    ...
    #Aba flight name split garney ani td ma chai sab xa tyo fares name sab.
  


f = YetiAir().search_for_flights('20','01','2022','KTM','BDP','1')
# f = Search('20','01','2022','KTM','BDP','1')
print(Fetch(f).get_flight_and_seats())
